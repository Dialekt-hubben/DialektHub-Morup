...
export const dialectWordTable = sqliteTable("dialect_word_table", {
  ...
  userId: int().references(() => userTable.id).notNull(),
  nationalWordId: int().references(() => nationalWordTable.id).notNull(),
  soundFileId: int().references(() => soundFileTable.id).notNull(),
});